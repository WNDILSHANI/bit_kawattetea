...
export class CategorizationTableComponent extends AbstractComponent implements OnInit {

  categorizationDataPage: CategorizationDataPage;
  ...
  async loadData(): Promise<any> {
    ...
    this.categorizationService.getAll(pageRequest).then((page: CategorizationDataPage) => {
      this.categorizationDataPage = page;
    }).catch( e => {
      console.log(e);
  ...
  async delete(categorization: Categorization): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: categorization.code}
    });

    dialogRef.afterClosed().subscribe( async result => {
      if (!result) { return; }
      try {
        await this.categorizationService.delete(categorization.id);
      }catch (e) {
