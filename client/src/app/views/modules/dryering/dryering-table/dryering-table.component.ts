...
@Component({
  selector: 'app-dryering-table',
  templateUrl: './dryering-table.component.html',
  styleUrls: ['./dryering-table.component.scss']
})
export class DryeringTableComponent extends AbstractComponent implements OnInit {

  dryeringDataPage: DryeringDataPage;
  ...
  async loadData(): Promise<any> {
    ...
    this.dryeringService.getAll(pageRequest).then((page: DryeringDataPage) => {
      this.dryeringDataPage = page;
    }).catch( e => {
      console.log(e);
  ...
  async delete(dryering: Dryering): Promise<void>{
    const dialogRef = this.dialog.open(DeleteConfirmDialogComponent, {
      width: '300px',
      data: {message: dryering.code}
    });

    dialogRef.afterClosed().subscribe( async result => {
      if (!result) { return; }
      try {
        await this.dryeringService.delete(dryering.id);
      }catch (e) {
